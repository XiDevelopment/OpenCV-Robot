...
public class PositionController {
	...
	public static double getAngle(double x, BeaconBounds bb, double robotX, double robotY) {
		
		//calculate bottomPointXPixel
		double beacon_x = bb.getGlobalCoordinate().x;
		double beacon_y = bb.getGlobalCoordinate().y;
		/*//rotate beacon to center		
		int center = 540;
		
		int difference1 =  bottomPointXPixel - center;	
		System.out.println(difference1);
		
		double rotate = (double)difference1 / (2 * (double)center) * (Math.PI / 4);
		System.out.println(Math.toDegrees(rotate));
		
		//MoveFacade.getInstance().turn(Math.toDegrees(rotate));
		
		//get new bottomPointXPixel???
		int bottomPointXPixelNew = 540;
				
		int difference2 = bottomPointXPixelNew - center;
		System.out.println(difference2);
		
		double alpha2 = rotate * (double) difference2 / ((double) difference1 - (double) difference2);
		System.out.println(Math.toDegrees(alpha2));	
		*/
		
				
		double c = calcDistance(robotX, robotY, beacon_x, beacon_y); //distance robot to beacon
		double a = 0.0; //distance: display middle to beacon in mm
		double b = 0.0; //distance to field boarder (depends on beacon)
		
		double alpha1 = 0.0; //degree normal (if beacon is exactly in the middle)
