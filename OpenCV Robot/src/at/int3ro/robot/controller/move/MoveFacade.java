...
public class MoveFacade {
	private static final String TAG = "Robot::MoveFacade";
	...
	/**
	 * Instantiates and starts a new thread that is waiting for the queue to be filled.
	 * The thread then automatically starts the instructions in the queue.
	 */
	public MoveFacade() {
		thread = new Thread() {
			@Override
	...
	/**
	 * Sets the context for our robot. That has to be done to build a connection.
	 * @param context
	 */
	...
	/**
	 * Returns a boolean if the MovementThread is working at the moment.
	 * True if it is working, false if it is sleeping.
	 * @return
	 */
	...
	/**
	 * Moves from the given Position and viewing angle to the specific target position.
	 * Calculates the angle between current and target.
	 * Rotates and then moves forward.
	 * @param curPos the current position
	 * @param curAngle the current angle we are looking at given that 0° is in the east
	...
	public void move(Point curPos, double curAngle, Point toPos) {
		...
		double distancePos = Math.sqrt(Math.pow(targetPos.x, 2.0)+Math.pow(targetPos.y, 2.0));
		
		//calculate angle between two vectors and convert the result into degrees
		...
		//if the angle is greater than 180, the turn is faster in the other direction
	...
	/**
	 * Moves the given centimeter in either direction.
	 * > 0 moves forward and < 0 moves backward.
	 * If cm is 0 the robot does nothing.
	 * @param cm the centimeter to move forward or backward
	 */
	...
	/**
	 * Turns the robot the given angle.
	 * > 0 rotates counter clockwise, < 0 rotates clockwise.
